...
class WatchContributors extends React.Component {
  ...
  toggleContributorsModal = watch => {
    this.setState({
      ...
      editContributorsFor: this.state.displayContributorsModal
        ? { id: "", name: "" }
        : watch
    });
  };
  ...
  render() {
    ...
    if (contributors) {
      _contributors =
        contributors.length > max ? contributors.slice(0, max) : contributors;
    }

    const remainingContributors =
      contributors && contributors.length > max ? contributors.length - max : 0;
    return (
      <div className={classNames("WatchContributors--wrapper flex-column", className)}>
        {title && (
          <div className="flex">
            ...
            <span
              ...
              onClick={() =>
                this.toggleContributorsModal({ id: watchId, name: watchName })
              }
            >
              Manage
            </span>
        ...
        <div className="flex-column">
          <p className="u-fontSize--normal u-fontWeight--bold u-color--tuna u-marginTop--15">
            Individuals
          </p>
          <div className="flex u-marginTop--15">
            {contributors &&
              contributors.length &&
              _contributors.map((contributor, i) => (
                <div
                  key={i}
                  className="watch-icon-wrapper u-cursor--pointer"
                  onClick={() =>
                    this.toggleContributorsModal({ id: watchId, name: watchName })
                  }
                  data-tip={`${contributor.login}-${i}`}
                  data-for={`${contributor.login}-${i}`}
                >
                  <span
                    className="contributer-icon"
                    style={{ backgroundImage: `url(${contributor.avatar_url})` }}
                  ></span>
                  <ReactTooltip
                    id={`${contributor.login}-${i}`}
                    effect="solid"
          ...
          {contributors && contributors.length > max ? (
            <div
              className="flex-column justifyContent--center u-cursor--pointer u-marginLeft--5 u-marginRight--5"
              onClick={() =>
                this.toggleContributorsModal({ id: watchId, name: watchName })
              }
            >
              <span className="u-color--tundora u-fontSize--small u-fontWeight--medium">
                + {remainingContributors} other{remainingContributors === 1 ? "" : "s"}
              </span>
            </div>
          ) : null}
          <div className="flex-column flex1 justifyContent--center u-marginLeft--5"></div>
        </div>
        {this.state.displayContributorsModal && (
          <WatchContributorsModal
            displayContributorsModal={this.state.displayContributorsModal}
            toggleContributorsModal={() => {
              this.toggleContributorsModal();
            }}
