...
class DiffGitHubReleases extends React.Component {
  ...
  render() {
    ...
    const gitPath =
      getWatch.getWatch.cluster && getWatch.getWatch.cluster.gitOpsRef
        ? `${getWatch.getWatch.cluster.gitOpsRef.owner}/${getWatch.getWatch.cluster.gitOpsRef.repo}/pull`
        : "";
        ...
        <div className="diff-header-wrapper u-marginTop--30">
          ...
          <div className=" u-marginTop--10">
            <span className="u-fontSize--large u-color--dustyGray u-fontWeight--medium">
              ...
              (
              <a
                href={`https://github.com/${gitPath}/${firstDiff.pullrequestNumber}`}
                target="_blank"
                rel="noopener noreferrer"
                className="replicated-link u-fontSize--meidum"
              >
                #{firstDiff.pullrequestNumber}
              </a>
              ) from{" "}
              <span className="u-fontSize--large u-color--doveGray u-fontWeight--bold">
                {secondDiff.title}
              ...
              (
              <a
                href={`https://github.com/${gitPath}/${secondDiff.pullrequestNumber}`}
                target="_blank"
                rel="noopener noreferrer"
                className="replicated-link u-fontSize--meidum"
              >
                #{secondDiff.pullrequestNumber}
              </a>
              )
            </span>
          </div>
