...
class VersionHistory extends React.Component {
  ...
  onCardChecked = (sequence, isChecked) => {
    if (isChecked) {
      this.setState({
        checkedCards: [{ sequence, isChecked }]
          .concat(this.state.checkedCards)
          .slice(0, 2)
      });
    } else {
      this.setState({
  ...
  render() {
    ...
    if (versionHistory) {
      ...
      if (checkedCards.length === 2) {
        ...
        const second = versionHistory.find(
          result => result.sequence === secondSeqNumber
        );
      ...
      return (
        <div className="flex1 flex-column u-position--relative verison-history-wrapper u-overflow--auto">
          <div className="version-card-header-wrapper">
                ...
                <div className="flex1 u-marginLeft--10">
                  <div className="flex flex-column">
                    <p className="u-fontSize--header2 u-fontWeight--bold u-color--tundora flex alignContent--center alignItems--center">
                      {watch.cluster ? (
                        <span
                          className={`icon u-marginRight--10 clusterType ${
                            gitOpsRef ? "git" : "ship"
                          }`}
                        ></span>
                      ) : (
                    ...
                    <span className="u-fontSize--large u-color--dustyGray u-fontWeight--medium u-marginTop--10">
                      Version history for{" "}
                      {gitOpsRef
                        ? `${gitOpsRef.owner}/${gitOpsRef.repo}/${gitOpsRef.branch}${gitOpsRef.path}`
                        : "your cluster"}
                    </span>
                  </div>
                </div>
            ...
            <div className="u-marginTop--30 u-paddingLeft--10 u-marginBottom--10 u-fontSize--header u-fontWeight--bold u-color--tundora">
              {watch.currentVersion && watch.currentVersion.title}
            </div>
            <div className="icon-wrapper flex-auto flex flex1 justifyContent--spaceBetween u-paddingLeft--10 u-marginTop--10">
              <div className="flex1 flex">
                ...
                <span className="flex1 flex-column flex-verticalCenter u-fontSize--normal u-color--dustyGray u-fontWeight--medium">
                  Most recent version
                </span>
              </div>
              {!selected ? (
                <div className="flex1 flex justifyContent--flexEnd">
                  <span
                    className="u-fontSize--normal u-fontWeight--medium u-color--astral u-textDecoration--underlineOnHover alignSelf--center"
                    onClick={() => this.onReleasesToDiff()}
                  >
                ...
                <div className="flex1 flex justifyContent--flexEnd">
                  <span
                    className="u-fontSize--normal u-fontWeight--medium u-color--astral u-textDecoration--underlineOnHover alignSelf--center u-marginRight--20"
                  ...
                  {checkedCards.length === 2 ? (
                    gitOpsRef ? (
                      <Link
                        to={`${this.props.match.url}/compare/${gitOpsRef.owner}/${gitOpsRef.repo}/${gitOpsRef.branch}${gitOpsRef.path}/${firstSeqNumber}/${secondSeqNumber}`}
                        className="btn primary alignSelf--center"
                      >
                        Diff releases
                      </Link>
                    ) : (
                      <Link
                        to={`${this.props.match.url}/compare/${firstSeqNumber}/${secondSeqNumber}`}
                        className="btn primary alignSelf--center"
                      >
                        Diff releases
                      </Link>
                    ...
                    <Link
                      to={`${this.props.match.url}/compare/${firstSeqNumber}/${secondSeqNumber}`}
                      className="btn primary disabled-link alignSelf--center"
                    >
                      Diff releases
                    </Link>
                  )}
                ...
                if (gitOpsRef) {
                  card = (
                    <GitHubVersionCard
                      ...
                      isPending={watch.pendingVersions.find(
                        version => version.sequence === gi.sequence
                      )}
                      versionHistory={gi}
                      pullRequestRootUrl={`${gitOpsRef.owner}/${gitOpsRef.repo}`}
                      onCardChecked={(sequence, isChecked) =>
                        this.onCardChecked(sequence, isChecked)
                      }
                      isChecked={
                        !!this.state.checkedCards.find(
                          card => card.sequence === gi.sequence
                        )
                      }
                    />
                  );
...
export default compose(
  ...
  graphql(deployWatchVersion, {
    props: ({ mutate }) => ({
      deployWatchVersion: (watchId, sequence) =>
        mutate({ variables: { watchId, sequence } })
    })
  })
)(VersionHistory);
