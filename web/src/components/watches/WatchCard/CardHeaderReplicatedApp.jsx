...
const CardHeaderReplicatedApp = ({
  ...
  <div className="installed-watch-header flex">
    <div className="installed-watch-name flex alignItems--center">
      ...
      ></div>
      <h2 className="u-fontSize--jumbo u-fontWeight--bold u-color--tuna">
        {watch.watchName}
      </h2>
    </div>
    ...
    {watchIntegrations &&
    (watchIntegrations.email.length || watchIntegrations.webhook.length) ? (
      <div className="installed-watch-integrations flex flex-column">
        ...
        <div className="flex alignItems--center">
          ...
          <Link
            to={`/watch/${watch.slug}/integrations`}
            className="u-marginLeft--10 u-fontSize--small replicated-link"
          >
            Manage
          </Link>
        </div>
    ...
    <div className="installed-watch-actions alignItems--center flex">
      {!isPending && (
        <Link
          to={`/watch/${watch.slug}`}
          className="u-marginRight--10 u-fontSize--small replicated-link"
        >
          Application Details
        </Link>
      )}
      <button className="btn secondary green" onClick={() => onEditApplication(watch)}>
        {isPending ? "Install application" : "Edit application"}
      </button>
    </div>
  </div>
);
