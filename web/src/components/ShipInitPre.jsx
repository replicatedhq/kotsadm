...
export class ShipInitPre extends React.Component {
  ...
  componentDidUpdate(lastProps, lastState) {
    ...
    if (
      this.state.displayLicenseIdModal !== lastState.displayLicenseIdModal &&
      this.state.displayLicenseIdModal
    ) {
      setTimeout(() => {
  ...
  onShipInitUrlSubmitted = async url => {
    ...
    if (url.indexOf("?license_id=") === -1 && url.indexOf("replicated.app") !== -1) {
      // Prompt user to input license ID before trying to fetch app
    ...
    const validUpstream = await this.validateUpstream(url).catch(
    ...
    if (validUpstream) {
      ...
      this.props
        .createInitSession(pendingInitId, url, clusterId, githubPath)
        .then(({ data }) => {
    ...
    if (this.state.saving) {
      return (
        <div className="Login-wrapper container flex-column flex1 u-overflow--auto">
          <ShipLoading
            headerText={`Fetching ${Utilities.toTitleCase(readableAppString) || ""}`}
            subText="We're moving as fast as we can but it may take a moment."
          />
        </div>
      );
    }
      ...
      <div className="Login-wrapper container flex-column flex1 u-overflow--auto">
          ...
          <div className="init-pre-wrapper flex-auto">
            <div className="flex1 flex-column u-textAlign--center">
              <p className="u-fontSize--larger u-color--tuna u-fontWeight--bold u-lineHeight--normal">
                What is the URL of the application you want to install?
              </p>
              <p className="u-fontSize--normal u-color--dustyGray u-lineHeight--normal u-marginBottom--30">
                This could be a URL to any Helm Chart, Kubernetes or Replicated
                application
              </p>
              {fetchError && (
                <p className="u-fontSize--small u-color--chestnut u-marginBottom--5 u-paddingBottom--20">
                  There's a problem with fetching {Utilities.toTitleCase(appString)}.
                  Verify you have the correct URL and try again
                </p>
              )}
              {urlError && (
                <p className="u-fontSize--small u-color--chestnut u-marginBottom--5 u-paddingBottom--20">
                  Please enter a valid url to the kubernetes application to continue
                </p>
              )}
              <div className="flex flex1">
                <input
                  ref={this.initFieldInputRef}
                  value={this.state.url}
                  onChange={e => {
                    this.onUrlChange(e.target.value);
                  }}
                  type="text"
                  className="Input jumbo flex1"
                  placeholder="https://github.com/helm/charts/stable/grafana"
                />
                <div className="flex-auto u-marginLeft--10">
                  <button
                    onClick={this.onShipInitUrlSubmitted.bind(this, this.state.url)}
            ...
            <div className="u-marginTop--10">
              <p className="u-fontSize--small u-color--dustyGray u-lineHeight--normal u-marginBottom--30">
                If you already have a <code>state.json</code> file you can{" "}
                <Link to="/watch/create/state" className="replicated-link">
                  upload it here
                </Link>
              </p>
            </div>
            ...
            <div className="flex flex1">
              <div className="unfork-callout flex-column flex1 flex">
                <div className="flex alignItems--center">
                  ...
                  <div className="flex1 justifyContent--center">
                    <p className="u-marginLeft--10 u-fontSize--normal u-fontWeight--medium u-lineHeight--normal u-color--tuna">
                      {" "}
                      Unfork existing apps.
                    </p>
                  </div>
                </div>
                <p className="u-marginTop--10 u-color--dustyGray  u-fontSize--small u-fontWeight--medium u-lineHeight--normal">
                  Have you already forked a 3rd-party Helm chart or k8s yaml?
                </p>
                <div className="flex alignItems--center">
                  <Link
                    to="/watch/create/unfork"
                    className="unforkLink u-marginTop--10 u-fontSize--small u-fontWeight--medium u-color--chateauGreen"
                  >
                    Let us unfork it for you{" "}
                    <span className="arrow icon clickable u-arrow u-marginLeft--5"></span>
                  </Link>
                </div>
              </div>
              ...
              <div className="unfork-callout flex-column flex1 flex">
                <div className="flex alignItems--center">
                  ...
                  <div className="flex1 justifyContent--center">
                    <p className="u-marginLeft--10 u-fontSize--normal u-fontWeight--medium u-lineHeight--normal u-color--tuna">
                      Not sure what to manage?
                    </p>
                  </div>
                </div>
                <p className="u-marginTop--10 u-color--dustyGray u-fontSize--small u-fontWeight--medium u-lineHeight--normal">
                  Try ClusterScope to discover the outdated images in your cluster.
                </p>
                <div className="flex alignItems--center">
                  <Link
                    to="/clusterscope"
        ...
        >
          <div className="flex-column licenseId-prompt-modal u-modalPadding">
            <p className="u-lineHeight--normal u-fontSize--larger u-color--tuna u-fontWeight--bold">
              License ID
            </p>
            <p className="u-lineHeight--normal u-fontSize--normal u-color--dustyGray u-fontWeight--normal u-marginTop--5">
              You must provide a license ID to fetch this app
            </p>
            <div className="u-marginTop--15">
              <input
                ref={this.licenseIdInputRef}
                value={this.state.licenseId}
                onChange={e => {
                  this.onLicenseIdChange(e.target.value);
                }}
                type="text"
...
export default compose(
  ...
  graphql(createInitSession, {
    props: ({ mutate }) => ({
      createInitSession: (pendingInitId, upstreamUri, clusterID, githubPath) =>
        mutate({ variables: { pendingInitId, upstreamUri, clusterID, githubPath } })
    })
  })
)(ShipInitPre);
