...
export class WatchContributorsModal extends React.Component {
  ...
  componentDidMount() {
    ...
    if (
      getGithubUserOrgs &&
      getGithubUserOrgs !== getGithubUserOrgs.installationOrganizations
    ) {
      if (getGithubUserOrgs.installationOrganizations) {
        const filteredOrgs = getGithubUserOrgs.installationOrganizations.installations.filter(
          org => this.props.getUserInfo.userInfo.login !== org.login
        );
  ...
  componentDidUpdate(lastProps) {
    const { getGithubUserOrgs, getWatchContributorsQuery } = this.props;
    if (
      getWatchContributorsQuery !== lastProps.getWatchContributorsQuery &&
      getWatchContributorsQuery.watchContributors
    ) {
      const { watchContributors } = getWatchContributorsQuery;
      this.setInitialState(watchContributors);
    }
    if (
      getGithubUserOrgs !== lastProps.getGithubUserOrgs &&
      getGithubUserOrgs.installationOrganizations
    ) {
      if (getGithubUserOrgs.installationOrganizations) {
        const filteredOrgs = getGithubUserOrgs.installationOrganizations.installations.filter(
          org => this.props.getUserInfo.userInfo.login !== org.login
  ...
  updateActiveContributors() {
    ...
    const _nextActive = omitBy(merge(activeContributors, contributors), {
      isActive: false
    });
  ...
  render() {
    ...
    const loading =
      (getUserInfo && this.props.getUserInfo.loading) ||
      (getWatchContributorsQuery && getWatchContributorsQuery.loading);
      ...
      >
        <div className="Modal-body flex flex-column flex1 u-overflow--auto">
          <h2 className="u-fontSize--largest u-fontWeight--bold u-color--tuna u-marginBottom--10">
            Edit contributors for {watchBeingEdited.name}
          </h2>
          <p className="u-fontSize--normal u-color--dustyGray u-lineHeight--normal u-marginBottom--30">
            Select the users that are allowed to make changes to this application. If
            you don't see who your looking for, head over to GitHub and add the desired
            user(s) to your org and they will be visible here.
          </p>
          <div className="Form">
            <div className="flex flex1 u-position--relative u-marginBottom--20">
              <div>
                ...
                <p className="u-fontSize--small u-color--dustyGray u-lineHeight--normal u-marginBottom--10">
                  Select the organization you want to add users from.
                </p>
                <Select
            ...
            <div>
              <p className="u-fontWeight--medium u-fontSize--small">
              ...
              <div className="flex flex-column u-borderTop--gray u-marginTop--10 u-position--relative">
                <div className="contributer-wrapper">
                  {!isEmpty(contributors) &&
                    Object.keys(contributors).map((key, i) => (
                      <WatchContributorCheckbox
                        key={i}
                        item={contributors[key]}
                        contributors={contributors}
                        handleCheckboxChange={(field, e) =>
                          this.handleCheckboxChange(field, e)
                        }
                        githubLogin={
                          getUserInfo && getUserInfo.userInfo.username.toLowerCase()
                        }
                      />
                    ))}
                </div>
                <div className="more-contributors flex justifyContent--center u-paddingBottom--10 u-borderTop--gray">
                  {nextPageMembers.length > 0 ? (
                    <button
                      className="btn secondary green u-marginTop--20"
                      disabled={savingContributors}
                      onClick={this.goNextOrgMemberPage}
                    >
                      Load more
                    </button>
                  ) : null}
            ...
            <div className="flex flex1 justifyContent--flexEnd alignItems--center u-marginTop--20">
              {showSaved && (
                <p className="u-fontSize--small u-color--chateauGreen u-marginRight--20 u-fontWeight--medium">
                  Contributors have been updated
                </p>
              )}
              <button
              ...
              <button
                ...
                disabled={savingContributors}
                onClick={this.onSaveContributors}
              >
                {savingContributors ? "Saving" : "Save contributors"}
              </button>
            </div>
          </div>
        </div>
