...
class AddNewClusterModal extends React.Component {
  ...
  render() {
    ...
    if (this.props.listClustersQuery && this.props.listClustersQuery.listClusters) {
      options = this.props.listClustersQuery.listClusters
        .filter(cluster => !existingDeploymentClusters.includes(cluster.id))
        .map(cluster => {
          return {
            value: cluster.id,
            label: cluster.title,
    ...
    const buttonDisabled =
      ...
      (this.state.selectedCluster.type === "git" && this.state.githubPath === "");
    return (
      <div className="flex flex1">
        <div className="flex-column">
          <p className="u-fontWeight--bold u-fontSize--normal u-color--tundora">
            Deployment clusters
          </p>
          <p className="u-fontWeight--medium u-fontSize--small u-color--dustyGray u-marginTop--5 u-lineHeight--medium">
            Select a cluster you would like to add for deployments.
          </p>
          <div className="u-marginTop--10">
            <Select
              ...
              isOptionSelected={option => {
                option.value === this.state.selectedCluster.value;
              }}
            />
          </div>
          {this.state.selectedCluster.type === "git" ? (
            <div className="u-marginTop--10">
              <p className="u-fontSize--normal u-color--tuna u-fontWeight--bold u-lineHeight--normal u-marginBottom--5">
                What is the root path for this cluster
              </p>
              <input
                type="text"
                className="Input"
                ...
                defaultValue={this.state.githubPath}
                onChange={e => {
                  this.setState({ githubPath: e.target.value });
                }}
              />
            </div>
          ) : null}
