...
export class SetUpEmailModal extends React.Component {
  ...
  onSaveClick = () => {
    ...
    if (notificationId) {
      if (isValid) {
        this.props
          .updateNotification(watchId, notificationId, null, email)
          .then(() => {
      ...
      if (isValid) {
        this.props
          .createNotification(watchId, null, email)
          .then(() => {
            this.setState({ isLoading: true });
            if (typeof this.props.submitCallback === "function") {
              this.props.submitCallback();
  ...
  componentDidMount() {
    if (this.props.notificationId) {
      this.props.client
        ...
        .then(res => {
          const resValue =
            res.data.getNotification.email.recipientAddress === "placeholder"
              ? ""
              : res.data.getNotification.email.recipientAddress;
  ...
  render() {
    const { emailError, emailToastMessage } = this.state;

    const { show, toggle, appName } = this.props;
    ...
    const content = this.state.isLoading ? (
      ...
      <div className="Modal-body flex flex-column flex1 u-overflow--auto">
        <h2 className="u-fontSize--largest u-color--tuna u-fontWeight--bold u-lineHeight--normal">
          Configure email notifications for {appName}
        </h2>
        <p className="u-fontSize--normal u-color--dustyGray u-lineHeight--normal u-marginBottom--20">
          Ship will send an email when an update is available.
        </p>
        <div className="Form">
          <div className="flex flex1 u-marginBottom--30">
            <div className="flex flex1 flex-column u-marginRight--10">
              ...
              <p className="u-fontSize--small u-color--dustyGray u-lineHeight--normal u-marginBottom--10">
                What's the address to send the email to?
              </p>
              {emailToastMessage && (
                <p className="u-fontSize--small u-color--chestnut u-marginBottom--5">
                  {emailToastMessage}
                </p>
              )}
              <input
                value={this.state.recipient}
                onChange={this.onChange.bind(this, "recipient")}
                type="text"
                className={`Input ${emailError ? "has-error" : "valid"}`}
          ...
          <div className="flex flex1 justifyContent--flexEnd u-marginTop--20">
            <div className="flex flex1 justifyContent--flexEnd u-marginTop--20">
              <button onClick={this.onSaveClick.bind(this)} className="btn primary">
                Schedule Emails
              </button>
            </div>
          </div>
...
export default compose(
  ...
  graphql(createNotification, {
    props: ({ mutate }) => ({
      createNotification: (watchId, webhook, email) =>
        mutate({ variables: { watchId, webhook, email } })
    })
  }),
  graphql(updateNotification, {
    props: ({ mutate }) => ({
      updateNotification: (watchId, notificationId, webhook, email) =>
        mutate({ variables: { watchId, notificationId, webhook, email } })
    })
  })
)(SetUpEmailModal);
