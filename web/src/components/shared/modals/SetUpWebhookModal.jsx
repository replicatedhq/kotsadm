...
export class SetUpWebhookModal extends React.Component {
  ...
  onSaveClick = () => {
    ...
    if (notificationId) {
      if (isValid) {
        this.props
          .updateNotification(watchId, notificationId, webhook, null)
          .then(() => {
      ...
      if (isValid) {
        this.props
          .createNotification(watchId, webhook, null)
          .then(() => {
            this.setState({ isLoading: true });
            if (typeof this.props.submitCallback === "function") {
              this.props.submitCallback();
  ...
  componentDidMount() {
    if (this.props.notificationId) {
      this.props.client
        ...
        .then(res => {
          const resValue =
            res.data.getNotification.webhook.uri === "placeholder"
              ? ""
              : res.data.getNotification.webhook.uri;
  ...
  render() {
    const { webhookUriError, webhookToastMessage } = this.state;
      ...
      >
        <div className="Modal-body flex flex-column flex1 u-overflow--auto">
          <h2 className="u-fontSize--largest u-color--tuna u-fontWeight--bold u-lineHeight--normal">
            Configure webhooks for {appName}
          </h2>
          <p className="u-fontSize--normal u-color--dustyGray u-lineHeight--normal u-marginBottom--20">
            Ship will trigger a webhook when an update is available.
          </p>
          <div className="Form">
              ...
              <div className="flex flex1">
                <div className="flex flex1 flex-column u-marginRight--10">
                  ...
                  <p className="u-fontSize--small u-color--dustyGray u-lineHeight--normal u-marginBottom--10">
                    What's the URI to send the webhook request to?
                  </p>
                  {webhookToastMessage && (
                  ...
                  <input
                    ...
                    onChange={this.onChange.bind(this, "uri")}
                    type="text"
                    className={`Input ${webhookUriError ? "has-error" : "valid"}`}
