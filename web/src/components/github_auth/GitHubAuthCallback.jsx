...
class GitHubAuthCallback extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (code && state) {
      await this.props
        .createGithubAuthToken(code, state)
        .then(res => {
          if (Utilities.localStorageEnabled()) {
            window.localStorage.setItem(
              "token",
              res.data.createGithubAuthToken.access_token
            );
            this.getUser();
          }
        })
      ...
      <div className="container flex-column flex1">
        {this.state.isLoading ? (
          <div className="flex-column flex1 u-overflow--auto justifyContent--center alignItems--center">
            ...
            <p className="u-fontSize--large u-fontWeight--bold u-marginTop--normal u-color--tuna">
              Authorizing GitHub
            </p>
          </div>
        ) : (
          <div className="flex-column flex1 u-overflow--auto justifyContent--center alignItems--center">
            ...
            <Link
              ...
              className="u-marginTop--30 btn primary"
              to={`/login`}
            >
              Back to log in
            </Link>
