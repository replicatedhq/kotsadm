...
const ShipUnforkError = ({ handleTryAgain }) => (
  <div className="Form flex-column flex1 alignItems--center justifyContent--center">
    ...
    <h2 className="u-fontSize--larger u-color--tuna u-fontWeight--bold u-lineHeight--normal u-marginTop--20 u-marginBottom--10">
      We were unable to unfork your application
    </h2>
    <p className="u-fontSize--normal u-color--dustyGray u-lineHeight--normal u-marginBottom--30 u-textAlign--center">
      We weren't able to unfork this application. We're aware that not all applications
      can be unforked currently and are working on solutions. <br />
      In the meantime, you can either try again or prepare your application for
      deployment with{" "}
      <span className="u-fontWeight--medium u-color--chateauGreen">Ship Init</span>.
    </p>
    <div className="flex">
      <button className="btn secondary u-marginRight--10" onClick={handleTryAgain}>
        Try again
      </button>
      <Link className="btn primary" to="/watch/create/init">
        Use Ship Init
      </Link>
    </div>
  </div>
...
export class ShipUnfork extends React.Component {
  ...
  onShipInitUrlSubmitted = async () => {
    ...
    if (url === "" || fork === "") {
      ...
      if (validUpstream) {
        this.props
          .createUnforkSession(url, fork)
          .then(({ data }) => {
            if (!window.location.pathname.includes("/watch/create/unfork")) {
              return;
    ...
    if (this.state.saving) {
      return (
        <div className="Login-wrapper container flex-column flex1 u-overflow--auto">
          <ShipLoading
            headerText={`Unforking ${Utilities.toTitleCase(appString) || ""}`}
          ...
          <div className="init-unfork-wrapper flex-auto">
            <div className="flex1 flex-column u-textAlign--center">
              ...
              <div className="flex flex1 alignItems--flexEnd">
                ...
                <div className="flex-column flex1 alignItems--flexStart">
                  ...
                  <input
                    defaultValue={this.state.fork}
                    value={this.state.fork}
                    onChange={e => {
                      this.onForkChange(e.target.value);
                    }}
                    type="text"
                    className="Input jumbo flex1"
                    placeholder="https://github.com/username/fork"
                  />
                </div>
                <div className="flex-auto u-marginLeft--10">
                  <button
                    onClick={this.onShipInitUrlSubmitted}
            ...
            <div className="u-marginTop--30 u-textAlign--center">
              <p className="u-fontSize--small u-color--dustyGray u-lineHeight--normal u-marginBottom--30">
                ...
                <Link to="/watch/create/init" className="replicated-link">
                  create a new watch here
                </Link>
              </p>
            </div>
          </div>
...
export default compose(
  ...
  graphql(createUnforkSession, {
    props: ({ mutate }) => ({
      createUnforkSession: (upstreamUri, forkUri) =>
        mutate({ variables: { upstreamUri, forkUri } })
    })
  })
)(ShipUnfork);
