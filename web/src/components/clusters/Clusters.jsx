...
export class Clusters extends React.Component {
  ...
  onUpdateCluster = async () => {
    ...
    await this.props
      .updateCluster(clusterToManage.id, clusterName, gitOpsRef)
      .then(() => {
  ...
  onDeleteCluster = async () => {
    ...
    await this.props
      .deleteCluster(clusterToManage.id)
      .then(() => {
        this.props.listClustersQuery.refetch();
        this.setState({
          displayDeleteClusterlModal: false,
  ...
  componentDidUpdate(lastProps, lastState) {
    if (
      this.state.displayInstallModal !== lastState.displayInstallModal &&
      ...
      <div className="ClusterDashboard--wrapper container flex-column flex1 u-overflow--auto">
              ...
              !listClustersQuery.listClusters.length ? (
                <div className="flex-column flex1 justifyContent--center alignItems--center">
                  <div className="EmptyState flex-column flex1 alignItems--center justifyContent--center">
                    <div className="u-textAlign--center u-marginTop--10">
                      ...
                      <p className="u-fontSize--more u-color--dustyGray u-lineHeight--medium u-fontWeight--medium">
                        You haven't connected any deployment pipelines yet. Ship can
                        deploy directly to a cluster or by pushing to a GitHub repo and
                ...
                <div className="u-flexTabletReflow flex-auto installed-clusters-wrapper flexWrap--wrap">
                  {listClustersQuery.listClusters.map((cluster, i) => (
                    <div
                      key={cluster.id}
                      className="installed-cluster-wrapper flex flex-auto u-paddingBottom--20"
                    >
                      <ClusterCard
                        index={i}
                        item={cluster}
                        ...
                        toggleInstallShipModal={() =>
                          this.toggleInstallShipModal(cluster)
                        }
                        toggleDeleteClusterModal={() =>
                          this.toggleDeleteClusterModal(cluster)
                        }
                      />
                    </div>
                  ))}
        ...
        {this.state.displayManageModal && (
          <Modal
            ...
            ariaHideApp={false}
            className={`ManageClusterModal--wrapper Modal ${
              clusterToManage.gitOpsRef ? "MediumSize" : "DefaultSize"
            }`}
          >
            <div className="Modal-body">
              <h2 className="u-fontSize--largest u-color--tuna u-fontWeight--bold u-lineHeight--normal">
                Update {clusterToManage.title} cluster
              </h2>
              <div className="flex flex1">
                <div className="flex-column flex1">
                  <p className="u-fontWeight--bold u-fontSize--normal u-color--tundora u-marginBottom--10 u-marginTop--10">
                    Cluster name
                  </p>
                  <input
                    type="text"
                  ...
                  <div
                    className={`u-marginTop--${
                      clusterToManage.gitOpsRef ? "20" : "10"
                    } u-paddingTop--5 flex`}
                  >
                    <button
                      onClick={() =>
                    ...
                    <button
                      disabled={
                        !this.state.clusterName.length || this.state.updatingCluster
                      }
                      onClick={this.onUpdateCluster}
        ...
        {this.state.displayInstallModal && (
          ...
          >
            <div className="Modal-body">
              <h2 className="u-fontSize--largest u-color--tuna u-fontWeight--bold u-lineHeight--normal">
                Install {this.state.clusterName}
              </h2>
              <div className="flex-column flex1">
                <p className="u-fontWeight--bold u-fontSize--normal u-color--tundora u-marginTop--10">
                  Run this command to install your cluster
                </p>
                <code className="u-lineHeight--normal u-fontSize--small u-overflow--auto">
                  kubectl apply -f{" "}
                  {`${window.env.INSTALL_ENDPOINT}/${this.state.clusterId}/${this.state.clusterToken}`}
                </code>
                <div className="HiddenText-wrapper u-position--absolute">
                  <textarea
                    value={`kubectl apply -f ${window.env.INSTALL_ENDPOINT}/${this.state.clusterId}/${this.state.clusterToken}`}
                    id="copy-ship-install-snippet"
                    readOnly={true}
                  ></textarea>
                </div>
                {Clipboard.isSupported() ? (
                  <span
                    className={`copy-ship-install-snippet u-fontSize--small u-fontWeight--medium u-marginBottom--10`}
                    data-clipboard-target={`#copy-ship-install-snippet`}
                  >
                    {this.state.shipInstallSnippetCopySuccess ? (
                      <span className="u-color--chateauGreen u-userSelect--none u-cursor--default">
        ...
        {this.state.displayDeleteClusterlModal && (
          ...
          >
            <div className="Modal-body">
              <h2 className="u-fontSize--large u-color--tuna u-fontWeight--bold u-lineHeight--normal">
                Are you sure you want to delete {this.state.clusterToManage.title}
              </h2>
              <div className="flex-column flex1">
                <p className="u-fontWeight--medium u-fontSize--normal u-color--dustyGray u-lineHeight--more u-marginTop--10">
                  You cannot undo this action. Clusters that have applications deployed
                  to them cannot be deleted.
                </p>
                {this.state.deleteErr && this.state.deleteErr.length && (
                  <p className="u-fontWeight--medium u-fontSize--normal u-color--red u-lineHeight--more u-marginTop--10">
...
export default compose(
  ...
  graphql(updateCluster, {
    props: ({ mutate }) => ({
      updateCluster: (clusterId, clusterName, gitOpsRef) =>
        mutate({ variables: { clusterId, clusterName, gitOpsRef } })
    })
  }),
